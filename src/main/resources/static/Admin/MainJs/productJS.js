...
app.controller("ctrlProduct", function($scope, $http) {
	...
	$scope.load_all = function() {
		...
		$http.get(`${hostProduct}`).then(resp => {
			...
			$scope.items.forEach(item => {
				...
				item.brand.name = decodeURIComponent(item.brand.name); // Giải mã tên thương hiệu
			});
		});
		...
		$http.get(urlbrand).then(resp => {
			$scope.brands = resp.data.map(brand => {
				return {
					...brand,
					name: decodeURIComponent(brand.name) // Giải mã tên thương hiệu
				};
			});
		});
	...
	$scope.create = function() {
		...
		if (!name) {
			Swal.fire(
				'Error',
				'Vui lòng chọn một ảnh để tạo sản phẩm.',
				'error'
			);
